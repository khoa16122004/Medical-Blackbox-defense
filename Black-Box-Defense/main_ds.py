...
import torch
import itertools
from custom_loss import MSE_CE_loss
...
# Dataset
parser.add_argument('--dataset', type=str, default="Brain_Tumor", choices=DATASETS)
...
def main():
    ...
    if args.dataset == 'cifar10' or args.dataset == 'stl10' or args.dataset == 'mnist':
        ...
        train_loader = DataLoader(train_dataset, shuffle=True, batch_size=args.batch,
                                   pin_memory=pin_memory, num_workers=args.workers)
    ...
    if args.train_objective == 'classification':
        init_logfile(logfilename, f"{args.dataset} \t {args.classifier} \t {args.optimization_method} \t {args.arch} \t {args.optimization_method} Crossentropy - \n\epoch\ttime\tlr\ttrainloss\ttestloss\ttestAcc")
...
def train(loader: DataLoader, denoiser: torch.nn.Module, 
          criterion, 
        ...
        if args.optimization_method == 'FO':
            ...
            if args.zo_method == 'RGE':
                with torch.no_grad():
                    ...
                    # loss_0_ce = criterion.cal_ce_loss(recon_pre, original_pre)

                    # record original loss
                    # loss_0_ce_mean = loss_0_ce.mean()
                    loss_0_mean = loss_0.mean()

                    losses.update(loss_0_mean.item(), inputs.size(0))
                    # losses.update(loss_0_ce_mean.item(), inputs.size(0))
                    ...
                    for k in range(args.q):
                        ...
                        u = torch.normal(m, sigma, size=(batch_size, d)).cuda()
                        u_norm = torch.norm(u, p=2, dim=1).reshape(batch_size, 1).expand(batch_size, d).cuda()    # dim -- careful
                        ...
                        # loss_tmp_ce_mean = criterion.cal_ce_loss(recon_q_pre, original_pre)
                        # loss_diff = torch.tensor(loss_tmp_ce_mean - loss_0_ce_mean)
                ...
                # loss = criterion.cal_mse_loss(recon, inputs).mean() + 0.1 * torch.sum(recon_flat * grad_est_no_grad, dim=-1).mean()
